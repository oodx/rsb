#!/usr/bin/env bash
# Generated by fx-testsh v2.0.0-proto
# Wed Sep 17 07:05:00 PM MDT 2025
...
_emit_override_warning() {
    ...
    _emit_boxy "$warning_text" "warning" "⚠️  Test Organization Override"
}
...
_run_header() {
    ...
    local content
    printf -v content '🚀 Running test lane\n\nLane    : %s\nProfile : %s\nMode    : %s\nStrict  : %s\nOverride: %s' \
        "$label" "$profile" "$mode" "$strict_state" "$override_state"

    _emit_boxy "$content" "$theme" "🧪 fx-testsh"
}
...
_with_env() {
    ...
    if [[ $# -eq 0 ]]; then
        __stderr "❌ _with_env requires '--' delimiter"
        return 1
    fi
...
_run_exec() {
    ...
    __stderr "❌ Unable to locate lane target for '$lane'"
...
_run_module() {
    ...
    if [[ -z "$mod" ]]; then
        __stderr "❌ Module name required for lane '$lane'"
        return 1
    fi
    ...
    __stderr "❌ Module filtering not supported: lane '$lane', module '$mod'"
    return 1
}
...
_run_lane_rsb() {
    ...
    case "$lane" in
        ...
        smoke)
            _run_header "$lane"
            local -a smoke_lanes=(sanity global param)
            local smoke status=0
            for smoke in "${smoke_lanes[@]}"; do
                if ! _run_lane "$smoke"; then
                    status=$?
                    break
                fi
            done
...
_run_dynamic() {
    ...
    if [[ -n "$mod" ]]; then
        __stderr "❌ Unknown lane '$lane' for module '$mod'"
        return 1
    fi
...
_run_adhoc() {
    ...
    if [[ ! -d "$dir" ]]; then
        __stderr "❌ Adhoc directory missing: ${dir#$TESTSH_PROJECT_ROOT/}"
        return 1
    fi

    if [[ -z "$name" ]]; then
        _emit_boxy "$(_adhoc_summary)" "info" "🧪 Adhoc"
        return 0
    fi
    ...
    if [[ -z "$path" ]]; then
        __stderr "❌ Adhoc test not found: $name"
        return 1
    fi

    __stderr "🧪 Running adhoc test: $name"
    __stderr "📁 ${path#$TESTSH_PROJECT_ROOT/}"
    __stderr ""
    ...
    case "$path" in
        ...
        *.sh)
            ...
            ;;
        *)
            __stderr "❌ Unsupported adhoc target: $path"
            return 1
            ;;
    esac
...
_adhoc_summary() {
    local dir="$TESTSH_TEST_ROOT/_adhoc"
    local msg="🧪 Adhoc tests\n\n"

    if [[ ! -d "$dir" ]]; then
        msg+="  • directory missing (tests/_adhoc)\n"
        printf '%s' "$msg"
        return 0
    fi
...
_load_profile() {
    ...
    case "$prof" in
        ...
        default|generic)
            ;;
        *)
            __stderr "⚠️ Unknown profile: $prof"
            ;;
    esac
}

_profile_rsb() {
    TESTSH_TEST_MAP[sanity]="sanity_main"
    TESTSH_TEST_MAP[param]="features_param"
    TESTSH_TEST_MAP["param-helpers"]="param_helpers"
    TESTSH_TEST_MAP["param-uat"]="uat_main"
    TESTSH_TEST_MAP[macros]="macro-smoke"
    TESTSH_TEST_MAP[context]="uat_global"
    TESTSH_TEST_MAP[global]="uat_global"
    TESTSH_TEST_MAP["host-env"]="uat_host_env"
    TESTSH_TEST_MAP["host-paths"]="uat_host_paths"
    TESTSH_TEST_MAP[cli]="sh/cli_macros_e2e"
    TESTSH_TEST_MAP[args]="args-processing"
    TESTSH_TEST_MAP[stdopts]="stdopts"
    TESTSH_TEST_MAP["uat-colors"]="uat_main"
    TESTSH_TEST_MAP["uat-colors-macros"]="uat_main"
    TESTSH_TEST_MAP["uat-stdopts"]="uat_stdopts"
    TESTSH_TEST_MAP["uat-glyphs"]="uat_glyphs"
    TESTSH_TEST_MAP["uat-visual"]="uat_visual"
    TESTSH_TEST_MAP["uat-prompts"]="uat_prompts"
    TESTSH_TEST_MAP[threads]="threads_sanity"
    TESTSH_TEST_MAP["uat-threads"]="uat_threads"
    TESTSH_TEST_MAP[bash]="bash_sanity"
    TESTSH_TEST_MAP["uat-bash"]="uat_bash"
    TESTSH_TEST_MAP[colors]="features_colors"
    TESTSH_TEST_MAP["colors-runtime"]="features_colors"
    TESTSH_TEST_MAP[bootstrap]="bootstrap-lifecycle"
    TESTSH_TEST_MAP[integration]="rsb-integration"
    TESTSH_TEST_MAP[regression]="regression-suite"
    TESTSH_TEST_MAP[defects]="known-defects"
    TESTSH_TEST_MAP[all]="all-tests"
    TESTSH_TEST_MAP[smoke]="smoke-tests"
    TESTSH_TEST_MAP[full]="comprehensive-suite"
    TESTSH_TEST_MAP[old]="old/legacy-tests"
...
_docs_overview() {
    ...
    if [[ -z "$root" || ! -d "$root" ]]; then
        __stderr "❌ Documentation root not found"
        return 1
    fi

    local msg="📚 Documentation Hub\n\nProfile: ${TESTSH_PROFILE:-default}\nRoot: ${root#$TESTSH_PROJECT_ROOT/}\n\nSections:\n"
    local dir
    ...
    _emit_boxy "$msg" "info" "📚 Documentation"
}
...
_docs_show() {
    ...
    if ! path=$(_docs_resolve "$topic"); then
        __stderr "❌ Documentation topic not found: $topic"
        return 1
    fi

    local title
    title="📄 ${path#$TESTSH_PROJECT_ROOT/}"
...
_docs_seed_default() {
    ...
    if [[ -f "$root/bashfx/BASHFX-v3.md" ]]; then
        TESTSH_DOC_OVERRIDE[bashfx]="$root/bashfx/BASHFX-v3.md"
        TESTSH_DOC_OVERRIDE[legendary]="$root/bashfx/BASHFX-v3.md"
    fi

    if [[ -f "$root/reference/BASHFX-v3.md" ]]; then
        TESTSH_DOC_OVERRIDE[bashfx_ref]="$root/reference/BASHFX-v3.md"
    fi
...
_org_validate() {
    ...
    __stderr "🔍 Validating test structure..."
    ...
    if [[ $total -gt 0 ]]; then
        if [[ "${TESTSH_FLAGS[violations]}" == "true" ]]; then
            __stderr "📋 Test Organization Violations Report ($total total)"
            __stderr "================================================================"
            __stderr ""

            if [[ ${#arr_name[@]} -gt 0 ]]; then
                __stderr "🏷️  NAMING VIOLATIONS (${#arr_name[@]} files)"
                __stderr "────────────────────────────────────────────────────────"
                ...
                __stderr ""
                __stderr "Fix: Rename files to match pattern (e.g., com_sanity.rs → sanity_com.rs)"
                __stderr ""
            fi

            if [[ ${#arr_sanity[@]} -gt 0 ]]; then
                __stderr "🚨 MISSING SANITY TESTS (${#arr_sanity[@]} modules)"
                __stderr "────────────────────────────────────────────────────────"
            ...
            if [[ ${#arr_uat[@]} -gt 0 ]]; then
                __stderr "🎭 MISSING UAT TESTS (${#arr_uat[@]} modules)"
                __stderr "────────────────────────────────────────────────────────"
            ...
            if [[ ${#arr_entry[@]} -gt 0 ]]; then
                __stderr "📋 MISSING CATEGORY ENTRY FILES (${#arr_entry[@]} categories)"
                __stderr "────────────────────────────────────────────────────────"
            ...
            if [[ ${#arr_root[@]} -gt 0 ]]; then
                __stderr "🚫 UNAUTHORIZED ROOT FILES (${#arr_root[@]} files)"
                __stderr "────────────────────────────────────────────────────────"
            ...
            if [[ ${#arr_dir[@]} -gt 0 ]]; then
                __stderr "📁 INVALID DIRECTORIES (${#arr_dir[@]} directories)"
                __stderr "────────────────────────────────────────────────────────"
        ...
        if [[ "${TESTSH_FLAGS[strict]}" == "true" && "${TESTSH_FLAGS[override]}" != "true" ]]; then
            ...
            _emit_boxy "$warn_msg" "warning" "⚠️  Organization Violations (Override Active)"
        else
            __stderr "⚠️  Test structure warnings ($total total):"
            __stderr "   • Naming issues: ${#arr_name[@]}"
            __stderr "   • Missing sanity tests: ${#arr_sanity[@]}"
            __stderr "   • Missing UAT tests: ${#arr_uat[@]}"
            __stderr "   • Missing category entries: ${#arr_entry[@]}"
            __stderr "   • Unauthorized root files: ${#arr_root[@]}"
            __stderr "   • Invalid directories: ${#arr_dir[@]}"
...
do_status() {
    ...
    local theme="success"
    local result_text="✅ organization compliant"
    if ((res != 0)); then
        theme="warning"
        result_text="⚠️  violations detected"
    fi

    local msg
    printf -v msg '🧭 fx-testsh status\n\nProject : %s\nProfile : %s\nMode    : %s\nTests   : %d\nDocs    : %s\nStrict  : %s\nOverride: %s\nResult  : %s\n' \
...
do_lint() {
    ...
    __stderr "🧹 Linting test organization..."
    __stderr ""
    ...
    if _org_validate "lint"; then
        __stderr "✅ Test organization lint completed"
...
do_run() {
    ...
    local theme="success"
    local result_text="✅ run completed"
    if ((status != 0)); then
        theme="error"
        result_text="❌ run failed (exit $status)"
    elif [[ "$override_active" == true ]]; then
        theme="warning"
        result_text="⚠️  completed under override"
    fi
    ...
    _emit_boxy "$summary" "$theme" "🏁 Run Summary"
...
do_list() {
    ...
    local msg="🗂️ Available test lanes\n\n"
    ...
    _emit_boxy "$msg" "info" "🗂️ Test Lanes"
}
