...
#[cfg(feature = "prompts")]
use rsb::{confirm, ask, select, prompt, confirm_timeout, ask_timeout, select_timeout, prompt_timeout};
...
fn main() {
    ...
    #[cfg(feature = "prompts")]
    {
        ...
        println!("4. prompt!(\"confirm\", \"Ready?\") → {}", prompt!("confirm", "Ready?"));
        println!("5. prompt!(\"ask\", \"City\", \"NYC\") → {}", prompt!("ask", "City", "NYC"));
        ...
        println!("6. confirm_timeout!(\"Deploy?\") → {}", confirm_timeout!("Deploy?"));
        println!("7. ask_timeout!(\"API Key\", \"test-key\") → {}", ask_timeout!("API Key", "test-key"));
        println!("8. select_timeout!(\"Environment\", &[\"dev\", \"prod\"], 0) → {}",
                select_timeout!("Environment", &["dev", "prod"], 0));

        println!("\n=== Timeout-Enhanced Macros (Explicit Timeout) ===");

        // Test explicit timeout overrides
        println!("9. confirm_timeout!(\"Force update?\", 1, true) → {}",
                confirm_timeout!("Force update?", 1, true));
        println!("10. ask_timeout!(\"Username\", \"admin\", 1) → {}",
                ask_timeout!("Username", "admin", 1));
        println!("11. select_timeout!(\"Protocol\", &[\"http\", \"https\"], 1, 1) → {}",
                select_timeout!("Protocol", &["http", "https"], 1, 1));

        println!("\n=== General Timeout Prompt Macro ===");

        println!("12. prompt_timeout!(\"confirm\", \"Backup first?\") → {}",
                prompt_timeout!("confirm", "Backup first?"));
        println!("13. prompt_timeout!(\"ask\", \"Email\", \"user@example.com\", 1) → {}",
        ...
        println!("14. confirm_timeout!(\"Final check?\") → {}",
        ...
        println!("CLI flag timeout: {}, Env timeout: {}",
                get_var("opt_prompt_timeout"), get_var("PROMPT_TIMEOUT"));
        println!("15. confirm_timeout!(\"Priority test?\", 1) → {} (explicit 1s wins)",
