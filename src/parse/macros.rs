...
macro_rules! sed_read {
    ($path:expr, FIRST, LAST) => { $crate::parse::sed_read($path, None, None) };
    ($path:expr, FIRST, $end:expr) => { $crate::parse::sed_read($path, None, Some($end as usize)) };
    ($path:expr, $start:expr, LAST) => { $crate::parse::sed_read($path, Some($start as usize), None) };
    ($path:expr, $start:expr, $end:expr) => { $crate::parse::sed_read($path, Some($start as usize), Some($end as usize)) };
}

// --- File-based Sed Macros (adapter; conceptually optional if fs is enabled) ---
#[macro_export]
macro_rules! sed_lines_file { ($path:expr, $start:expr, $end:expr) => { $crate::parse::sed_lines_file($path, $start, $end) }; }

#[macro_export]
macro_rules! sed_around_file { ($path:expr, $pattern:expr, $context:expr) => { $crate::parse::sed_around_file($path, $pattern, $context) }; }
...
#[macro_export]
macro_rules! sed_template_file { ($path:expr, $content:expr, $sentinel:expr) => { $crate::parse::sed_template_file($path, $content, $sentinel) }; }
