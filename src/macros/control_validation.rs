...
pub use crate::{for_in, test, case, file_in, export, src, load_config, validate, require_file, require_dir, require_command, require_var};
#[macro_export]
macro_rules! for_in {
...
macro_rules! test {
    ...
    ($a:expr, ==, $b:expr) => { $crate::string::str_equals($a, $b) };
    ($a:expr, !=, $b:expr) => { !$crate::string::str_equals($a, $b) };
    ($a:expr, =~, $b:expr) => { $crate::string::str_matches($a, $b) };
    ($a:expr, <, $b:expr) => { $a < $b };
    ($a:expr, >, $b:expr) => { $a > $b };
...
macro_rules! export {
    ...
    ($path:expr) => { $crate::global::export_vars($path); };
}
