...
pub fn confirm_default(message: &str, default: bool) -> bool {
    if is_true("opt_yes") { return true; }
    if is_true("opt_quiet") { return default; }
    if !stdin_is_tty() { return default; }

    let suffix = match default { true => " [Y/n]", false => " [y/N]" };
...
pub fn confirm(message: &str) -> bool {
    if is_true("opt_yes") { return true; }
    if is_true("opt_quiet") { return false; }
...
pub fn ask(message: &str, default: Option<&str>) -> String {
    ...
    if trimmed.is_empty() { default.unwrap_or("").to_string() } else { trimmed.to_string() }
}
...
pub fn select(message: &str, options: &[&str], default_index: Option<usize>) -> String {
    ...
    let idx_default = default_index.unwrap_or(0).min(options.len().saturating_sub(1));
    ...
    let opts_inline = options
        ...
        .enumerate()
        .map(|(i, o)| {
            if i == idx_default { format!("{{yellow}}[{}]{}{{reset}}", i + 1, o) } else { format!("[{}]{}", i + 1, o) }
        })
        .collect::<Vec<_>>()
        .join("  ");
    ...
    loop {
        ...
        let t = input.trim();
        if t.is_empty() { return options[idx_default].to_string(); }
