...
pub fn ns_set(ns: &str, key: &str, value: &str) {
    crate::global::set_var(make_key(ns, key, NsStyle::Dunder), value);
}

/// Set a namespaced key (colon-colon style: NS::KEY)
pub fn ns_set_cc(ns: &str, key: &str, value: &str) {
    crate::global::set_var(make_key(ns, key, NsStyle::Colon), value);
}
...
pub fn ns_get_all(ns: &str) -> Vec<(String, String)> {
    ...
    for (k, v) in crate::global::get_all_vars() {
        if let Some(rest) = k.strip_prefix(&dunder_prefix) {
            map.insert(rest.to_string(), v.clone());
        }
        if let Some(rest) = k.strip_prefix(&colon_prefix) {
            map.entry(rest.to_string()).or_insert(v.clone());
        }
    }
