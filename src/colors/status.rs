...
pub fn get_status_color_categories() -> Vec<(&'static str, Vec<&'static str>)> {
    vec![
        (
            "Missing BashFX Colors",
            vec!["magic", "trace", "note", "silly"],
        ),
        (
            "Error/Alert Status",
            vec!["error", "warning", "danger", "alert", "fatal"],
        ),
        (
            "Success/Positive Status",
            vec!["success", "complete", "verified", "approved", "passed"],
        ),
        (
            "Info/Neutral Status",
            vec!["info", "debug", "hint", "notice"],
        ),
        (
            "Process/State Status",
            vec![
                "pending", "progress", "blocked", "queued", "active", "inactive", "paused",
                "stopped",
            ],
        ),
        (
            "Additional BashFX Status",
            vec!["recover", "think", "okay", "warn", "uclock"],
        ),
    ]
}
