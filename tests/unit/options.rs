...
fn long_with_value_sets_value_and_validates_paths() {
    ...
    let args = vec!["rsb-test".to_string(), format!("--input-file={}", p)];
    let args = rsb::cli::Args::new(&args);
    options!(&args);
...
fn comma_list_values_remain_raw_for_caller_to_split() {
    let args = vec!["rsb-test".to_string(), "--features=a,b,c".to_string()];
    let args = rsb::cli::Args::new(&args);
    options!(&args);
...
fn not_prefix_alone_sets_negative_marker_only() {
    let args = vec!["rsb-test".to_string(), "--not-trace".to_string()];
    let args = rsb::cli::Args::new(&args);
    options!(&args);
...
fn multi_generic_sets_letters_and_supports_inline_and_commas() {
    ...
    let args = vec!["rsb-test".to_string(), "--multi=d,q".to_string()];
    let args = rsb::cli::Args::new(&args);
    options!(&args);
    ...
    let args2 = vec!["rsb-test".to_string(), "--multi=dq!ts".to_string()];
