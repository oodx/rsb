...
fn uat_colors_basic_demo() {
    ...
    for name in basics.iter() {
        println!("{:>10}: {}sample{}", name, color(name), color("reset"));
    }
    let status = [
        "magic", "trace", "note", "silly", "error", "warning", "success", "info",
    ];
    println!("\n-- Status Colors --");
    for name in status.iter() {
        println!("{:>10}: {}status{}", name, color(name), color("reset"));
    }
    let named = [
        "crimson", "emerald", "azure", "amber", "violet", "aqua", "slate", "gold",
    ];
    println!("\n-- Named Colors --");
    for name in named.iter() {
        println!("{:>10}: {}named{}", name, color(name), color("reset"));
    }
    ...
    for name in basics.iter() {
        let b = bg(name);
        if !b.is_empty() {
            println!("{:>10}: {} bg {} ", name, b, color("reset"));
        }
    }
    println!(
        "\n{}BG sample{} {}\n",
        bg("crimson"),
        colorize_bg("with bg", "amber"),
        color("reset")
    );
