...
fn host_global_hydrate_and_ns() {
    ...
    rsb::hosts::host_global::hydrate_env_and_configs(&[tmp.to_str().unwrap()]);
    ...
    rsb::hosts::host_global::import_env_with_prefix("MYAPP_", true);
    assert_eq!(rsb::global::get_var("FOO"), "bar");
    assert_eq!(rsb::global::get_var("NUM"), "7");

    // Namespacing helpers
    rsb::hosts::host_global::ns_set("ns1", "name", "value");
    assert_eq!(rsb::hosts::host_global::ns_get("ns1", "name"), "value");
    let all = rsb::hosts::host_global::ns_get_all("ns1");
